...
pub fn get_or_create_session_for_user_isolates_by_user_key_test() {
  with_db(fn(db) {
    ...
    let assert Ok(id1) =
      db.get_or_create_session_for_user(db, user_key)
    let assert Ok(id2) =
      db.get_or_create_session_for_user(db, "telegram:22222")
...
pub fn messages_from_completed_session_still_accessible_test() {
  with_db(fn(db) {
    ...
    let assert Ok(session_id) =
      db.get_or_create_session_for_user(db, user_key)
    let assert Ok(Nil) = db.save_chat_message(db, session_id, "user", "Hello from first session")
    let assert Ok(Nil) = db.save_chat_message(db, session_id, "assistant", "Reply 1")
...
pub fn get_recent_messages_returns_correct_order_test() {
  with_db(fn(db) {
    ...
    let assert Ok(Nil) = db.save_chat_message(db, session_id, "user", "only msg")
